...
class SalidaAlmacen
{
    ...
    // ================================================================
    // PICKING — POOL + COMPARAR + CONFIRMAR (nuevo flujo)
    // ================================================================

    /**
     * Compara el pool de códigos escaneados contra las necesidades del albarán.
     * No realiza ninguna escritura en BD.
     *
     * @param int   $id_salida ID de salida_almacen
     * @param array $codigos   Array de strings (codigo_elemento)
     * @return array {correctos[], sobran[], no_relacionados[], faltan[]} | ['error' => string]
     */
    public function comparar_pool(int $id_salida, array $codigos): array
    {
        try {
            // Q1: id_version_presupuesto de la salida
            $stmt1 = $this->conexion->prepare(
                "SELECT id_version_presupuesto FROM salida_almacen
                 WHERE id_salida_almacen = ? AND activo_salida_almacen = 1 LIMIT 1"
            );
            $stmt1->bindValue(1, $id_salida, PDO::PARAM_INT);
            $stmt1->execute();
            $salida = $stmt1->fetch(PDO::FETCH_ASSOC);
            if (!$salida) {
                return ['error' => 'Salida no encontrada'];
            }
            $id_version = (int)$salida['id_version_presupuesto'];

            // Q2: líneas del presupuesto con datos de familia
            $stmt2 = $this->conexion->prepare(
                "SELECT lp.id_linea_ppto, lp.id_articulo, lp.cantidad_linea_ppto, lp.id_ubicacion,
                        a.nombre_articulo, a.codigo_articulo,
                        f.id_familia, f.nombre_familia
                 FROM linea_presupuesto lp
                 INNER JOIN articulo a ON lp.id_articulo = a.id_articulo
            ...
            $stmt2->bindValue(1, $id_version, PDO::PARAM_INT);
            $stmt2->execute();
            $lineas_ppto = $stmt2->fetchAll(PDO::FETCH_ASSOC);

            // Construir mapa: id_articulo → {meta, lineas_libres[]}
            // Cada unidad requerida ocupa un slot en lineas_libres para asignarla 1-a-1
            $necesidades = [];
            foreach ($lineas_ppto as $lp) {
                $id_art = (int)$lp['id_articulo'];
                if (!isset($necesidades[$id_art])) {
                    $necesidades[$id_art] = [
                        'id_articulo'     => $id_art,
                        'nombre_articulo' => $lp['nombre_articulo'],
                        'id_familia'      => $lp['id_familia'],
                        'nombre_familia'  => $lp['nombre_familia'],
                        'lineas_libres'   => []
                    ];
                }
                for ($i = 0; $i < (int)$lp['cantidad_linea_ppto']; $i++) {
                    $necesidades[$id_art]['lineas_libres'][] = [
                        'id_linea_ppto' => (int)$lp['id_linea_ppto'],
                        'id_ubicacion'  => $lp['id_ubicacion']
                    ];
                }
            }

            // Q3: resolver cada código → elemento + artículo + familia (deduplicado)
            $codigos_unicos = array_values(array_unique(array_map('trim', $codigos)));
            $stmtE = $this->conexion->prepare(
                "SELECT e.id_elemento, e.codigo_elemento,
                        e.id_articulo_elemento AS id_articulo,
                        a.nombre_articulo, a.codigo_articulo,
                        ee.codigo_estado_elemento,
                        f.id_familia, f.nombre_familia
                 FROM elemento e
                 INNER JOIN articulo       a  ON e.id_articulo_elemento = a.id_articulo
                 INNER JOIN estado_elemento ee ON e.id_estado_elemento   = ee.id_estado_elemento
            ...
            foreach ($codigos_unicos as $codigo) {
                if ($codigo === '') continue;
                $stmtE->bindValue(1, $codigo, PDO::PARAM_STR);
                $stmtE->execute();
                $elem = $stmtE->fetch(PDO::FETCH_ASSOC);
                ...
                $id_art = (int)$elem['id_articulo'];
                $base   = [
                    'codigo_elemento'        => $codigo,
                    'id_elemento'            => (int)$elem['id_elemento'],
                    'id_articulo'            => $id_art,
                    'nombre_articulo'        => $elem['nombre_articulo'],
                    'id_familia'             => $elem['id_familia'],
                    'nombre_familia'         => $elem['nombre_familia'],
                    'codigo_estado_elemento' => $elem['codigo_estado_elemento'],
                    'estado_invalido'        => !in_array($elem['codigo_estado_elemento'], ['DISP', 'TERC'])
                ];

                if (!isset($necesidades[$id_art])) {
                    // Artículo no está en el albarán → no relacionado
                    $no_relacionados[] = $base;
                } elseif (!empty($necesidades[$id_art]['lineas_libres'])) {
                    // Hay unidades pendientes → correcto
                    $slot = array_shift($necesidades[$id_art]['lineas_libres']);
                    $correctos[] = array_merge($base, [
                        'id_linea_ppto' => $slot['id_linea_ppto'],
                        'id_ubicacion'  => $slot['id_ubicacion']
                    ]);
                } else {
                    // Artículo en albarán pero cantidad ya cubierta → sobra
                    // Asignamos id_linea_ppto de la primera línea del albarán para ese artículo
                    $id_linea_ppto_sobra = null;
                    foreach ($lineas_ppto as $lp) {
                        if ((int)$lp['id_articulo'] === $id_art) {
                            $id_linea_ppto_sobra = (int)$lp['id_linea_ppto'];
                            break;
                        }
                    }
                    $sobran[] = array_merge($base, ['id_linea_ppto' => $id_linea_ppto_sobra]);
                }
            }

            // Faltantes = slots que quedaron sin cubrir
            $faltan = [];
            foreach ($necesidades as $nec) {
                foreach ($nec['lineas_libres'] as $slot) {
                    $faltan[] = [
                        'id_articulo'     => $nec['id_articulo'],
                        'nombre_articulo' => $nec['nombre_articulo'],
                        'id_familia'      => $nec['id_familia'],
                        'nombre_familia'  => $nec['nombre_familia'],
                        'id_linea_ppto'   => $slot['id_linea_ppto'],
                        'id_ubicacion'    => $slot['id_ubicacion']
                    ];
                }
            ...
            $this->registro->registrarActividad(
                'admin', 'SalidaAlmacen', 'comparar_pool',
                "Pool comparado id_salida=$id_salida: " . count($correctos) . " correctos, " .
                count($sobran) . " sobran, " . count($no_relacionados) . " no_rel, " .
                count($faltan) . " faltan",
                'info'
            );

            return compact('correctos', 'sobran', 'no_relacionados', 'faltan');
            ...
            return ['error' => 'Error interno al comparar el pool'];
        }
    }

    /**
     * Confirma el pool: inserta líneas de salida y cambia estado → PREP.
     * Operación atómica. No escribe nada si hay faltantes (la validación es responsabilidad
     * del controller, que no debe llamar a este método si quedan faltantes sin cubrir).
     *
     * @param int   $id_salida ID de salida_almacen
     * @param array $pool      [{codigo_elemento, modo: correcto|backup|sustituye, id_linea_ppto?: int}]
     * @return bool
     */
    public function confirmar_pool(int $id_salida, array $pool): bool
    {
        try {
            $this->conexion->beginTransaction();

            // Orden de escaneo: continuar desde el último existente
            $stmtOrd = $this->conexion->prepare(
                "SELECT COALESCE(MAX(orden_escaneo), 0) AS max_ord FROM linea_salida_almacen WHERE id_salida_almacen = ?"
            );
            $stmtOrd->bindValue(1, $id_salida, PDO::PARAM_INT);
            $stmtOrd->execute();
            $orden = (int)$stmtOrd->fetch(PDO::FETCH_ASSOC)['max_ord'] + 1;

            $stmtE = $this->conexion->prepare(
                "SELECT e.id_elemento, e.id_articulo_elemento AS id_articulo
                 FROM elemento e
                 WHERE e.codigo_elemento = ? AND e.activo_elemento = 1 LIMIT 1"
            );
            $stmtIns = $this->conexion->prepare(
                "INSERT INTO linea_salida_almacen
                     (id_salida_almacen, id_elemento, id_articulo, id_linea_ppto,
                      es_backup_linea_salida, orden_escaneo, fecha_escaneo_linea_salida)
                 VALUES (?, ?, ?, ?, ?, ?, NOW())"
            );
            $stmtUbic = $this->conexion->prepare(
                "SELECT id_ubicacion FROM linea_presupuesto WHERE id_linea_ppto = ? LIMIT 1"
            );

            foreach ($pool as $item) {
                $codigo        = trim($item['codigo_elemento'] ?? '');
                $modo          = $item['modo'] ?? 'backup';
                $id_linea_ppto = isset($item['id_linea_ppto']) ? (int)$item['id_linea_ppto'] : null;
                ...
                $stmtE->execute();
                $elem = $stmtE->fetch(PDO::FETCH_ASSOC);
                if (!$elem) continue; // código inválido — saltar

                $es_backup = ($modo === 'backup') ? 1 : 0;

                $stmtIns->bindValue(1, $id_salida,                 PDO::PARAM_INT);
                $stmtIns->bindValue(2, (int)$elem['id_elemento'],  PDO::PARAM_INT);
                $stmtIns->bindValue(3, (int)$elem['id_articulo'],  PDO::PARAM_INT);
                $stmtIns->bindValue(4, $id_linea_ppto,             $id_linea_ppto !== null ? PDO::PARAM_INT : PDO::PARAM_NULL);
                $stmtIns->bindValue(5, $es_backup,                 PDO::PARAM_INT);
                $stmtIns->bindValue(6, $orden,                     PDO::PARAM_INT);
                $stmtIns->execute();
                $id_linea_salida = (int)$this->conexion->lastInsertId();
                $orden++;

                // Movimiento automático si la línea del presupuesto tiene ubicación
                if ($id_linea_ppto !== null) {
                    $stmtUbic->bindValue(1, $id_linea_ppto, PDO::PARAM_INT);
                    $stmtUbic->execute();
                    $ub = $stmtUbic->fetch(PDO::FETCH_ASSOC);
                    if ($ub && !empty($ub['id_ubicacion'])) {
                        $this->registrar_movimiento(
                            $id_linea_salida,
                            null,
                            (int)$ub['id_ubicacion'],
                ...
                // Estado → PREP
                $this->cambiar_estado_elemento((int)$elem['id_elemento'], 'PREP');
            }

            $this->conexion->commit();

            $this->registro->registrarActividad(
                'admin', 'SalidaAlmacen', 'confirmar_pool',
                "Pool confirmado id_salida=$id_salida: " . count($pool) . " elementos → PREP",
                'info'
            );
            ...
            $this->registro->registrarActividad(
                'admin', 'SalidaAlmacen', 'confirmar_pool',
                "Error (rollback): " . $e->getMessage(), 'error'
            );
            return false;
        }
    }
