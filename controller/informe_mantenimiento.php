...
switch ($_GET["op"]) {
    ...
    case "generar_pdf":
        try {
            ...
            $datos = $elemento->getMaintenanceEvents($month, $year);
...
<head>
    <meta charset="UTF-8">
    <title>Informe de Mantenimientos - ' . $nombreMes . ' ' . $year . '</title>
    <style>
        @page {
            size: A4;
            margin: 15mm;
...
<body>
    ...
    <div class="header">
        <h1>🔧 Informe de Mantenimientos</h1>
        ...
        <p class="date">Generado el ' . date('d/m/Y H:i') . '</p>
            ...
            if (count($datos) > 0) {
                ...
                foreach ($datos as $row) {
                    switch ($row['estado_mantenimiento_elemento']) {
                        case 'Al día':
                            $totalAlDia++;
                            break;
                        case 'Próximo':
    ...
    <div class="summary no-print">
        ...
        <div class="summary-item al-dia">
            <strong>Al día:</strong> ' . $totalAlDia . '
        </div>
        <div class="summary-item proximo">
            <strong>Próximos:</strong> ' . $totalProximo . '
        </div>
        <div class="summary-item atrasado">
            <strong>Atrasados:</strong> ' . $totalAtrasado . '
        </div>';
        
                if ($totalSinProgramar > 0) {
                    $html .= '
        <div class="summary-item sin-programar">
            <strong>Sin programar:</strong> ' . $totalSinProgramar . '
        </div>';
                }
    ...
    <table>
        ...
        <tbody>';
                ...
                foreach ($datos as $row) {
                    // Formatear fecha
                    $fecha = date('d/m/Y', strtotime($row['proximo_mantenimiento_elemento']));
            ...
            <tr>
                ...
                <td class="' . $estadoClase . '">' . htmlspecialchars($row['estado_mantenimiento_elemento']) . '</td>
            </tr>';
                }
                ...
                $html .= '
    <div class="no-data">
        <p>⚠️ No hay mantenimientos programados para ' . $nombreMes . ' de ' . $year . '</p>
    </div>';
            }
    ...
    <div class="footer">
        ...
        <p>Total de registros: ' . count($datos) . ' | Página generada: ' . date('d/m/Y H:i:s') . '</p>
            ...
            $registro->registrarActividad(
                'admin',
                'informe_mantenimiento.php',
                'generar_pdf',
                "PDF de mantenimientos generado para $nombreMes $year - Total registros: " . count($datos),
                'info'
            );
