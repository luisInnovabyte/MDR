...
<head>
    ...
    <style>
        ...
        @page {
            size: A4;
            margin: 5mm 10mm 15mm 10mm; /* Margen mínimo, thead maneja la cabecera */
        ...
        * {
        ...
        /* ============================================ */
        /* CABECERA REPETIDA EN IMPRESIÓN */
        /* ============================================ */
        .document-table {
            width: 100%;
        ...
        @media print {
            /* Forzar que thead se repita en cada página al imprimir */
            .document-table {
            ...
            .document-table tfoot {
                display: none !important; /* Ocultar en impresión */
            }
            
            /* Asegurar que la cabecera no se rompa */
            .header-section {
                page-break-inside: avoid !important;
                page-break-after: avoid !important;
            }
            
            /* No romper columnas del header */
            .columna-izquierda,
            .columna-derecha {
                page-break-inside: avoid !important;
            }
...
<body>
    ...
    <!-- Tabla estructural para repetir cabecera en cada página -->
    ...
    <!-- ============================================ -->
    <!-- HEADER PRINCIPAL (se repite en cada página) -->
    <!-- ============================================ -->
    ...
    $html .= '
        ...
        <tfoot>
            <tr>
                <td>
                    <!-- Numeración de páginas (solo visible en pantalla, CSS counters en impresión) -->
                    <div class="page-number no-print" style="text-align: right; font-size: 8pt; color: #636e72; margin-top: 5px;"></div>
                </td>
            </tr>
        </tfoot>
    </table><!-- fin document-table -->
    
    <!-- Script para numeración de páginas -->
    <script>
        // Calcular y mostrar número de páginas (solo en pantalla)
        function actualizarNumeroPagina() {
            var pageNumberEl = document.querySelector(\'.page-number\');
            if (pageNumberEl) {
                // Calcular páginas aproximadas
                // A4 height = 297mm = ~1122px con 96dpi
                // Menos márgenes superior (15mm) + inferior (15mm) = 30mm = ~113px
                // Altura útil por página ≈ 1009px
                var altoPagina = 1009;
                var tbody = document.querySelector(\'.document-table tbody\');
                if (tbody) {
                    var altoContenido = tbody.scrollHeight;
                    var totalPaginas = Math.ceil(altoContenido / altoPagina);
                    
                    // Mostrar en pantalla (el CSS @page se encarga de la impresión)
                    pageNumberEl.textContent = \'Documento de \' + totalPaginas + \' página\' + (totalPaginas > 1 ? \'s\' : \'\');
                }
            }
        }
        ...
        window.addEventListener(\'load\', function() {
            setTimeout(actualizarNumeroPagina, 200);
        });
