...
function _generar_pdf_anticipo(
    ...
    $pdf->Cell($w_iva,   $col_h, 'IVA %',        1, 0, 'C', true);
    $pdf->Cell($w_imp,   $col_h, 'Importe €',    1, 1, 'C', true);

    $pdf->SetFont('helvetica', '', 8);
    $pdf->SetTextColor(0, 0, 0);
    $desc_linea = 'A cuenta del presupuesto ' . ($datos_ppto['numero_presupuesto'] ?? '');
    ...
    $pdf->Cell($w_label,  5, 'IVA ' . $pct_iva . '%:', 0, 0, 'R');
    ...
    $pdf->Cell($w_value,  8, number_format($importe_total, 2, ',', '.') . ' €', 1, 1, 'R', true);
