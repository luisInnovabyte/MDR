$(document).ready(function () {
  ...
  //     FORMATEO DE CAMPOS          //
  ///////////////////////////////////
  // FormValidator removido - ahora se maneja en formularioEstadoElemento.js
  /////////////////////////////////////////
  //     FIN FORMATEO DE CAMPOS          //
  ////////////////////////////////////////

  /////////////////////////////////////
  // INICIO DE LA TABLA DE ESTADOS DE ELEMENTO //
  //         DATATABLES             //
  ///////////////////////////////////
  var datatable_estadosElementoConfig = {
    ...
    columns: [
      ...
      {
        name: "id_estado_elemento",
        data: "id_estado_elemento",
        visible: false,
        className: "text-center",
      }, // Columna 1: ID_ESTADO_ELEMENTO
      {
    ...
    columnDefs: [
      ...
      // Columna 1: id_estado_elemento
      {
        targets: "id_estado_elemento:name",
        width: "5%",
        searchable: false,
        orderable: false,
        className: "text-center",
      ...
      // Columna 2: codigo_estado_elemento
      {
        targets: "codigo_estado_elemento:name",
        width: "15%",
        searchable: true,
        orderable: true,
        className: "text-center",
      ...
      // Columna 3: descripcion_estado_elemento
      {
        targets: "descripcion_estado_elemento:name",
        width: "20%",
        searchable: true,
        orderable: true,
        className: "text-center",
      ...
      // Columna 4: color_estado_elemento
      {
        targets: "color_estado_elemento:name",
        width: "15%",
        searchable: true,
        orderable: true,
        className: "text-center",
        render: function (data, type, row) {
          if (type === "display") {
            return row.color_estado_elemento
              ? `<div class="d-flex align-items-center justify-content-center">
                   <span style="display: inline-block; background-color: ${row.color_estado_elemento}; width: 20px; height: 20px; border-radius: 50%; margin-right: 8px; border: 1px solid #ccc;"></span>
                   <span>${row.color_estado_elemento}</span>
                 </div>`
      ...
      // Columna 5: permite_alquiler_estado_elemento
      {
        targets: "permite_alquiler_estado_elemento:name",
        width: "10%",
        searchable: true,
        orderable: true,
        className: "text-center",
        render: function (data, type, row) {
          if (type === "display") {
            return row.permite_alquiler_estado_elemento == 1
              ? '<span class="badge bg-success">SÍ</span>'
              : '<span class="badge bg-danger">NO</span>';
          }
          return row.permite_alquiler_estado_elemento;
        },
      },
      // Columna 6: activo_estado_elemento (Estado)
      {
        targets: "activo_estado_elemento:name",
        width: "10%",
        orderable: true,
        searchable: true,
        className: "text-center",
        render: function (data, type, row) {
          if (type === "display") {
            return row.activo_estado_elemento == 1
              ? '<i class="bi bi-check-circle text-success fa-2x"></i>'
              : '<i class="bi bi-x-circle text-danger fa-2x"></i>';
          }
          return row.activo_estado_elemento;
        },
      },
      // Columna 7: BOTON PARA ACTIVAR/DESACTIVAR ESTADO
      {
        ...
        render: function (data, type, row) {
          ...
          if (row.activo_estado_elemento == 1) {
            // permito desactivar el estado de elemento
            return `<button type="button" class="btn btn-danger btn-sm desacEstadoElemento" data-bs-toggle="tooltip-primary" data-placement="top" title="Desactivar" data-original-title="Tooltip on top" 
                             data-id_estado_elemento="${row.id_estado_elemento}"> 
                             <i class="fa-solid fa-trash"></i>
                             </button>`;
          } else {
            // debo permitir activar de nuevo el estado de elemento
            return `<button class="btn btn-success btn-sm activarEstadoElemento" data-bs-toggle="tooltip-primary" data-placement="top" title="Activar" data-original-title="Tooltip on top" 
                             data-id_estado_elemento="${row.id_estado_elemento}">
      ...
      // Columna 8: BOTON PARA EDITAR ESTADO DE ELEMENTO
      {
        targets: "editar:name",
        width: "10%",
        searchable: false,
        ...
        render: function (data, type, row) {
          // El nombre que de la variable que se pasa por data-xxx debe ser el mismo que el nombre de la columna en la base de datos
          // botón editar el estado de elemento
          return `<button type="button" class="btn btn-info btn-sm editarEstadoElemento" data-toggle="tooltip-primary" data-placement="top" title="Editar"  
                             data-id_estado_elemento="${row.id_estado_elemento}"> 
  ...
  }; // de la variable datatable_estadosElementoConfig
  ////////////////////////////
  // FIN DE LA TABLA DE ESTADOS DE ELEMENTO //
  ///////////////////////////

  /************************************/
  //     ZONA DE DEFINICIONES        //
  /**********************************/
  // Definición inicial de la tabla de estados de elemento
  var $table =
    $(
      "#estados_elemento_data"
    ); /*<--- Es el nombre que le hemos dado a la tabla en HTML */
  var $tableConfig =
    datatable_estadosElementoConfig; /*<--- Es el nombre que le hemos dado a la declaración de la definicion de la tabla */
  //var $columSearch = 3; /* <-- Es la columna en la cual al hacer click el valor se colocará en la zona de search y se buscará */
  var $tableBody = $(
    "#estados_elemento_data tbody"
  ...
  var $columnFilterInputs = $(
    "#estados_elemento_data tfoot input, #estados_elemento_data tfoot select"
  ...
  //ejemplo -- var table_e = $('#estados_elemento-table').DataTable(datatable_estadosElementoConfig);
            ...
            <div class="card border-primary mb-3" style="overflow: visible;">
                <div class="card-header bg-primary text-white">
                    <div class="d-flex align-items-center">
                        ...
                        <h5 class="card-title mb-0">Detalles del Estado de Elemento</h5>
                    </div>
                </div>
                <div class="card-body p-0" style="overflow: visible;">
                    <table class="table table-borderless table-striped table-hover mb-0">
                        <tbody>
                            ...
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-flag me-2"></i>Descripción
                                </th>
                                <td class="pe-4">
                                    ${
                                      d.descripcion_estado_elemento ||
                            ...
                            <tr>
                                ...
                                <td class="pe-4">
                                    ${
                                      d.color_estado_elemento
                                        ? `<div class="d-flex align-items-center">
                                             <span class="badge" style="background-color: ${d.color_estado_elemento}; color: white; width: 30px; height: 30px; border-radius: 50%; margin-right: 10px;"></span>
                                             <span class="fw-bold">${d.color_estado_elemento}</span>
                                           </div>`
                            ...
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-check-circle me-2"></i>Permite Alquiler
                                </th>
                                <td class="pe-4">
                                    ${
                                      d.permite_alquiler_estado_elemento == 1
                                        ? '<span class="badge bg-success fs-6">SÍ permite alquilar elementos en este estado</span>'
                                        : '<span class="badge bg-danger fs-6">NO permite alquilar elementos en este estado</span>'
                                    }
                                    <br>
                                    <small class="text-muted mt-1">
                                        ${d.permite_alquiler_estado_elemento == 1
                                            ? 'Los elementos en este estado están disponibles para incluir en presupuestos y alquileres' 
                                            : 'Los elementos en este estado NO pueden ser alquilados a clientes'
                                        }
                                    </small>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="pe-4" style="max-width: 300px; word-wrap: break-word; white-space: pre-wrap; overflow-wrap: break-word;">
                                    ${
                                      d.observaciones_estado_elemento ||
  ...
  /////////////////////////////////////
  //   INICIO ZONA DELETE ESTADO ELEMENTO  //
  ///////////////////////////////////
  function desacEstadoElemento(id) {
    Swal.fire({
      title: "Desactivar",
      html: `¿Desea desactivar el estado de elemento con ID ${id}?<br><br><small class="text-warning"><i class="bi bi-exclamation-triangle me-1"></i>Este estado no estará disponible para nuevos elementos</small>`,
      icon: "question",
      ...
      if (result.isConfirmed) {
        $.post(
          "../../controller/estado_elemento.php?op=eliminar",
          ...
          function (data) {
            ...
            Swal.fire(
              "Desactivado",
              "El estado de elemento ha sido desactivado",
              "success"
            );
          }
        );
  ...
  $(document).on("click", ".desacEstadoElemento", function (event) {
    event.preventDefault();
    let id = $(this).data("id_estado_elemento");
    desacEstadoElemento(id);
  });
  ////////////////////////////////////
  //   FIN ZONA DELETE ESTADO ELEMENTO    //
  //////////////////////////////////

  ///////////////////////////////////////
  //   INICIO ZONA ACTIVAR ESTADO ELEMENTO  //
  /////////////////////////////////////
  function activarEstadoElemento(id) {
    Swal.fire({
      title: "Activar",
      text: `¿Desea activar el estado de elemento con ID ${id}?`,
      icon: "question",
      showCancelButton: true,
      confirmButtonText: "Si",
      cancelButtonText: "No",
      ...
      if (result.isConfirmed) {
        $.post(
          "../../controller/estado_elemento.php?op=activar",
          { id_estado_elemento: id },
          function (data) {
            $table.DataTable().ajax.reload();

            Swal.fire("Activado", "El estado de elemento ha sido activado", "success");
          }
        );
      }
    });
  ...
  $(document).on("click", ".activarEstadoElemento", function (event) {
    event.preventDefault();
    let id = $(this).data("id_estado_elemento");
    console.log("id estado elemento:", id);

    activarEstadoElemento(id);
  });
  ////////////////////////////////////
  //   FIN ZONA ACTIVAR ESTADO ELEMENTO    //
  //////////////////////////////////
  ...
  // *****************************************************/
  // FUNCIONES DE GUARDAR ESTADO ELEMENTO REMOVIDAS
  ...
  $(document).on("click", ".editarEstadoElemento", function (event) {
    event.preventDefault();
    ...
    window.location.href = `formularioEstadoElemento.php?modo=editar&id=${id}`;
  });
  ///////////////////////////////////////
  //        FIN ZONA EDITAR           //
  /////////////////////////////////////
