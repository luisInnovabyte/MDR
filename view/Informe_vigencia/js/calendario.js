// ========================================
// CALENDARIO DE VIGENCIAS - JavaScript
// ========================================

class CalendarioVigencias {
    constructor() {
    ...
    renderCalendar() {
        ...
        for (let i = 0; i < totalDays; i++) {
            ...
            dayElement.innerHTML = `
                ...
                <div class="day-events" data-date="${this.formatDate(actualDate)}">
                    <!-- Aquí se cargarán los eventos de garantías -->
                </div>
            `;
        ...
        // Cargar eventos de garantías
        this.loadWarrantyEvents();
    }
    ...
    onDayClick(date, isCurrentMonth) {
        ...
        // Aquí se implementará la lógica para mostrar detalles de garantías
        // por ahora solo mostramos en consola
    }
    
    // Método para cargar eventos de garantías
    loadWarrantyEvents() {
        console.log('Cargando eventos de garantías...');
        
        const formData = new FormData();
        formData.append('op', 'getWarrantyEvents');
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                this.displayWarrantyEvents(data.data);
            } else {
    ...
    displayWarrantyEvents(events) {
        console.log('Mostrando eventos:', events);
        
        if (!events || events.length === 0) {
            console.log('No hay eventos de garantías para este mes');
            return;
        }
        
        // Recorrer cada evento
        events.forEach(evento => {
            const fechaGarantia = evento.fecha_fin_garantia_elemento;
            const estadoGarantia = evento.estado_garantia_elemento;
            const codigoElemento = evento.codigo_elemento;
            
            // Buscar el contenedor del día correspondiente
            const dayContainer = document.querySelector(`.day-events[data-date="${fechaGarantia}"]`);
    ...
    showEventDetails(evento) {
        ...
        // Rellenar datos de garantía
        document.getElementById('modalFechaGarantia').textContent = evento.fecha_fin_garantia_elemento || '-';
        
        // Estado de garantía con badge de color
        const estadoElement = document.getElementById('modalEstadoGarantia');
        let badgeClass = 'badge-secondary';
        switch(evento.estado_garantia_elemento) {
            case 'Vencida':
        ...
        estadoElement.innerHTML = `<span class="badge ${badgeClass}">${evento.estado_garantia_elemento || '-'}</span>`;
    ...
    exportToPDF() {
        ...
        form.method = 'POST';
        form.action = '../../controller/informe_garantias.php?op=generar_pdf';
        form.target = '_blank';
        ...
        console.log(`Exportando PDF para ${monthName} ${year}`);
    }
}

// Inicializar el calendario cuando el DOM esté listo
document.addEventListener('DOMContentLoaded', function() {
    const calendario = new CalendarioVigencias();
