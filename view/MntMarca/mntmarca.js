$(document).ready(function () {
    ...
    var formValidator = new FormValidator('formMarca', {
        codigo_marca: {
            required: true
        },
        nombre_marca: {
    ...
    /////////////////////////////////////
    // INICIO DE LA TABLA DE MARCAS //
    //         DATATABLES             //
    ///////////////////////////////////
    var datatable_marcasConfig = {
        ...
        columnDefs: [
            ...
            // Columna 1: id_marca
            { targets: "id_marca:name", width: '5%', searchable: false, orderable: false, className: "text-center" },
            // Columna 2: codigo_marca
            { targets: "codigo_marca:name", width: '15%', searchable: true, orderable: true, className: "text-center" },
            // Columna 3: nombre_marca
            { targets: "nombre_marca:name", width: '20%', searchable: true, orderable: true, className: "text-center" },
            // Columna 4: descr_marca
            { targets: 4, className: "text-start" },
            // Columna 5: activo_marca (Estado)
            {
                targets: "activo_marca:name", width: '10%', orderable: true, searchable: true, className: "text-center",
                render: function (data, type, row) {
                    if (type === "display") {
                        return row.activo_marca == 1 ? '<i class="bi bi-check-circle text-success fa-2x"></i>' : '<i class="bi bi-x-circle text-danger fa-2x"></i>';
                    }
                    return row.activo_marca;
                }
            },
            ...
            {   
                ...
                render: function (data, type, row) {
                    ...
                    if (row.activo_marca == 1) {
                        // permito desactivar la marca
                        return `<button type="button" class="btn btn-danger btn-sm desacMarca" data-bs-toggle="tooltip-primary" data-placement="top" title="Desactivar" data-original-title="Tooltip on top" 
                             data-id_marca="${row.id_marca}"> 
                             <i class="fa-solid fa-trash"></i>
                             </button>`}
                    else {
                        // debo permitir activar de nuevo la marca
                        return `<button class="btn btn-success btn-sm activarMarca" data-bs-toggle="tooltip-primary" data-placement="top" title="Activar" data-original-title="Tooltip on top" 
                             data-id_marca="${row.id_marca}">
                             <i class="bi bi-hand-thumbs-up-fill"></i>
                            </button>`}
                } // de la function
            },// 
            // Columna 7: BOTON PARA EDITAR MARCA
            {   
                targets: "editar:name", width: '10%', searchable: false, orderable: false, class: "text-center",
                render: function (data, type, row) {
                    // El nombre que de la variable que se pasa por data-xxx debe ser el mismo que el nombre de la columna en la base de datos
                    // botón editar la marca
                    return `<button type="button" class="btn btn-info btn-sm editarMarca" data-toggle="tooltip-primary" data-placement="top" title="Editar"  
                             data-id_marca="${row.id_marca}"> 
    ...
    }; // de la variable datatable_marcasConfig
    ////////////////////////////
    // FIN DE LA TABLA DE MARCAS //
    ///////////////////////////
    ...
    /**********************************/
    // Definición inicial de la tabla de marcas
    var $table = $('#marcas_data');  /*<--- Es el nombre que le hemos dado a la tabla en HTML */
    var $tableConfig = datatable_marcasConfig; /*<--- Es el nombre que le hemos dado a la declaración de la definicion de la tabla */
    //var $columSearch = 3; /* <-- Es la columna en la cual al hacer click el valor se colocará en la zona de search y se buscará */
    var $tableBody = $('#marcas_data tbody'); /*<--- Es el nombre que le hemos dado al cuerpo de la tabla en HTML */
    /* en el tableBody solo cambiar el nombre de la tabla que encontraremos en HTML*/
    var $columnFilterInputs = $('#marcas_data tfoot input, #marcas_data tfoot select'); /*<--- Selecciona tanto inputs como selects de los pies de la tabla en HTML */
            ...
            <div class="card border-primary mb-3" style="overflow: visible;">
                <div class="card-header bg-primary text-white">
                    <div class="d-flex align-items-center">
                        ...
                        <h5 class="card-title mb-0">Detalles de la Marca</h5>
                    </div>
                </div>
                        ...
                        <tbody>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-hash me-2"></i>Id Marca
                                </th>
                                <td class="pe-4">
                                    ${d.id_marca || '<span class="text-muted fst-italic">No tiene id marca</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-tags me-2"></i>Nombre Marca
                                </th>
                                <td class="pe-4">
                                    ${d.nombre_marca || '<span class="text-muted fst-italic">No tiene nombre marca</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-tags me-2"></i>Nombre Marca (en)
                                </th>
                                <td class="pe-4">
                                    ${d.name_marca || '<span class="text-muted fst-italic">No tiene traducción (en)</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-chat-left-text me-2"></i>Observaciones Marca
                                </th>
                                <td class="pe-4" style="max-width: 300px; word-wrap: break-word; white-space: pre-wrap; overflow-wrap: break-word;">
                                    ${d.descr_marca || '<span class="text-muted fst-italic">No tiene observaciones</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-calendar-plus me-2"></i>Marca creada el:
                                </th>
                                <td class="pe-4">
                                    ${d.created_at_marca ? formatoFechaEuropeo(d.created_at_marca) : '<span class="text-muted fst-italic">Sin fecha</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-calendar-plus me-2"></i>Marca actualizada el:
                                </th>
                                <td class="pe-4">
                                  ${d.updated_at_marca ? formatoFechaEuropeo(d.updated_at_marca) : '<span class="text-muted fst-italic">Sin fecha</span>'}
                                </td>
                            </tr>
                        </tbody>
                    </table>
    ...
    /////////////////////////////////////
    //   INICIO ZONA DELETE MARCAS  //
    ///////////////////////////////////
    function desacMarca(id) {
        Swal.fire({
            title: 'Desactivar',
            html: `¿Desea desactivar la marca con ID ${id}?<br><br><small class="text-warning"><i class="bi bi-exclamation-triangle me-1"></i>Esto desactivará todos los artículos que tengan esta marca</small>`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.post("../../controller/marca.php?op=eliminar", { id_marca: id }, function (data) {

                    $table.DataTable().ajax.reload();

                    Swal.fire(
                        'Desactivado',
                        'La marca ha sido desactivada',
                        'success'
                    )
                });
            }
    ...
    $(document).on('click', '.desacMarca', function (event) {
        event.preventDefault();
        let id = $(this).data('id_marca');
        desacMarca(id);
    });
    ////////////////////////////////////
    //   FIN ZONA DELETE MARCA    //
    //////////////////////////////////

    ///////////////////////////////////////
    //   INICIO ZONA ACTIVAR MARCA  //
    /////////////////////////////////////
    function activarMarca(id) {
        Swal.fire({
            title: 'Activar',
            text: `¿Desea activar la marca con ID ${id}?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'Si',
            cancelButtonText: 'No',
            ...
            if (result.isConfirmed) {
                $.post("../../controller/marca.php?op=activar", { id_marca: id }, function (data) {

                    $table.DataTable().ajax.reload();

                    Swal.fire(
                        'Activado',
                        'La marca ha sido activada',
                        'success'
                    )
                });
            }
    ...
    $(document).on('click', '.activarMarca', function (event) {
        event.preventDefault();
        let id = $(this).data('id_marca');
    ...
    $(document).on('click', '#btnnuevo', function (event) {
        ...
        $('#mdltitulo').text('Nuevo registro de marca');
        console.log('Título del modal cambiado');

        // Verificar si el modal existe antes de mostrarlo
        if ($('#modalMarca').length === 0) {
            console.error('Error: El modal #modalMarca no se encontró en el DOM');
            return;
        }
        ...
        // RESETEAR ID SEGURO
        $('#formMarca').find('input[name="id_marca"]').val("");
        ...
        document.getElementById('modalMarca').addEventListener('shown.bs.modal', function () {
            document.getElementById('codigo_marca').focus();
        });
    });
...
//*****************************************************/
//   CAPTURAR EL CLICK EN EL BOTÓN DE SALVAR MARCA
//*****************************************************/

$(document).on('click', '#btnSalvarMarca', function (event) {
    event.preventDefault();

    // Obtener valores del formulario de forma más directa
    var id_marcaR = $('#formMarca').find('input[name="id_marca"]').val().trim();
    var codigo_marcaR = $('input[name="codigo_marca"]').val().trim();
    var nombre_marcaR = $('input[name="nombre_marca"]').val().trim();
    var name_marcaR = $('input[name="name_marca"]').val().trim();
    var descr_marcaR = $('textarea[name="descr_marca"]').val().trim();
    ...
    // Verificar marca primero
    verificarMarcaExistente(id_marcaR, codigo_marcaR, name_marcaR, nombre_marcaR, descr_marcaR);
});

function verificarMarcaExistente(id_marca, codigo_marca, name_marca, nombre_marca, descr_marca) {
    $.ajax({
        url: "../../controller/marca.php?op=verificarMarca",
        type: "GET",
        data: { 
        ...
        success: function(response) {
            if (!response.success) {
                toastr.warning(response.message || "Error al verificar la marca.");
                return;
            }

            if (response.existe) {
                mostrarErrorMarcaExistente(nombre_marca);
            } else {
                guardarMarca(id_marca, codigo_marca, name_marca, nombre_marca, descr_marca);
            }
        },
        error: function(xhr, status, error) {
            console.error('Error en verificación:', error);
            toastr.error('Error al verificar la marca. Intente nuevamente.', 'Error');
        }
    });
}

function mostrarErrorMarcaExistente(nombre_marca) {
    console.log("Marca duplicada detectada:", nombre_marca);
    Swal.fire({
        title: 'Nombre de marca duplicado',
        text: 'La marca "' + nombre_marca + '" ya existe. Por favor, elija otro nombre.',
        icon: 'warning',
        confirmButtonText: 'Entendido'
    });
}

function guardarMarca(id_marca, codigo_marca, name_marca, nombre_marca, descr_marca) {
    var datos = { 
        codigo_marca: codigo_marca,
        nombre_marca: nombre_marca,
        name_marca: name_marca,
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.success) {
                ...
                var modalMarca = bootstrap.Modal.getInstance(document.getElementById('modalMarca'));
                modalMarca.hide();
                table_e.ajax.reload(null, false);
                $("#formMarca")[0].reset();
                toastr.success(res.message || "Marca guardada correctamente");
            } else {
                toastr.error(res.message || "Error al guardar la marca");
            }
        },
        error: function(xhr, status, error) {
            console.error("Error en guardado:", error);
            Swal.fire('Error', 'No se pudo guardar la marca. Error: ' + error, 'error');
        }
    });
}
    ...
    $(document).on('click', '.editarMarca', function (event) {
        event.preventDefault();
        ...
        $.ajax({
            url: "../../controller/marca.php?op=mostrar",
            type: "POST",
            data: { id_marca: id },
            ...
            success: function(data) {
                try {
                    ...
                    $('#mdltitulo').text('Edición registro marca');
                    
                    // Llenamos los campos del formulario
                    $('#formMarca').find('input[name="id_marca"]').val(data.id_marca);
                    $('#formMarca input[name="codigo_marca"]').val(data.codigo_marca);
                    $('#formMarca input[name="nombre_marca"]').val(data.nombre_marca);
                    $('#formMarca input[name="name_marca"]').val(data.name_marca);
                    $('#formMarca textarea[name="descr_marca"]').val(data.descr_marca);
                    
                    // Configurar el switch de estado
                    $('#formMarca input[name="activo_marca"]').prop('checked', data.activo_marca == 1);
            ...
            error: function(xhr, status, error) {
                ...
                toastr.error('Error al obtener datos de la marca');
            }
        });
    });
    ///////////////////////////////////////
