$(document).ready(function () {
    ...
    //     FORMATEO DE CAMPOS          //
    ///////////////////////////////////
    // FormValidator removido - ahora se maneja en formularioImpuesto.js
    /////////////////////////////////////////
    ...
    /////////////////////////////////////
    // INICIO DE LA TABLA DE IMPUESTOS //
    //         DATATABLES             //
    ///////////////////////////////////
    var datatable_impuestosConfig = {
        ...
        columnDefs: [
            ...
            // Columna 1: id_impuesto 
            { targets: "id_impuesto:name", width: '5%', searchable: false, orderable: false, className: "text-center" },
            // Columna 2: tipo_impuesto
            { targets: "tipo_impuesto:name", width: '20%', searchable: true, orderable: true, className: "text-center" },
            // Columna 3: tasa_impuesto
            { targets: "tasa_impuesto:name", width: '15%', searchable: true, orderable: true, className: "text-center",
                render: function (data, type, row) {
                    if (type === "display") {
                        return row.tasa_impuesto + '%';
                    }
                    return row.tasa_impuesto;
                }
            },
            // Columna 4: activo_impuesto (Estado)
            {
                targets: "activo_impuesto:name", width: '10%', orderable: true, searchable: true, className: "text-center",
                render: function (data, type, row) {
                    if (type === "display") {
                        return row.activo_impuesto == 1 ? '<i class="bi bi-check-circle text-success fa-2x"></i>' : '<i class="bi bi-x-circle text-danger fa-2x"></i>';
                    }
                    return row.activo_impuesto;
                }
            },
            ...
            {   
                ...
                render: function (data, type, row) {
                    ...
                    if (row.activo_impuesto == 1) {
                        // permito desactivar el impuesto
                        return `<button type="button" class="btn btn-danger btn-sm desacImpuesto" data-bs-toggle="tooltip-primary" data-placement="top" title="Desactivar" data-original-title="Tooltip on top" 
                             data-id_impuesto="${row.id_impuesto}"> 
                             <i class="fa-solid fa-trash"></i>
                             </button>`;
                    } else {
                        // debo permitir activar de nuevo el impuesto
                        return `<button class="btn btn-success btn-sm activarImpuesto" data-bs-toggle="tooltip-primary" data-placement="top" title="Activar" data-original-title="Tooltip on top" 
                             data-id_impuesto="${row.id_impuesto}">
            ...
            // Columna 6: BOTON PARA EDITAR IMPUESTO
            {   
                targets: "editar:name", width: '10%', searchable: false, orderable: false, class: "text-center",
                render: function (data, type, row) {
                    // El nombre que de la variable que se pasa por data-xxx debe ser el mismo que el nombre de la columna en la base de datos
                    // botón editar el impuesto
                    return `<button type="button" class="btn btn-info btn-sm editarImpuesto" data-toggle="tooltip-primary" data-placement="top" title="Editar"  
                             data-id_impuesto="${row.id_impuesto}"> 
        ...
        ajax: {
            url: '../../controller/impuesto.php?op=listar',
            type: 'GET',
    ...
    }; // de la variable datatable_impuestosConfig
    ////////////////////////////
    // FIN DE LA TABLA DE IMPUESTOS //
    ///////////////////////////
    ...
    /**********************************/
    // Definición inicial de la tabla de impuestos
    var $table = $('#impuestos_data');  /*<--- Es el nombre que le hemos dado a la tabla en HTML */
    var $tableConfig = datatable_impuestosConfig; /*<--- Es el nombre que le hemos dado a la declaración de la definicion de la tabla */
    var $tableBody = $('#impuestos_data tbody'); /*<--- Es el nombre que le hemos dado al cuerpo de la tabla en HTML */
    /* en el tableBody solo cambiar el nombre de la tabla que encontraremos en HTML*/
    var $columnFilterInputs = $('#impuestos_data tfoot input, #impuestos_data tfoot select'); /*<--- Selecciona tanto inputs como selects de los pies de la tabla en HTML */
    /* en el $columnFilterInputs solo cambiar el nombre de la tabla que encontraremos en HTML*/

    //ejemplo -- var table_e = $('#impuestos-table').DataTable(datatable_impuestosConfig);
            ...
            <div class="card border-primary mb-3" style="overflow: visible;">
                <div class="card-header bg-primary text-white">
                    <div class="d-flex align-items-center">
                        ...
                        <h5 class="card-title mb-0">Detalles del Impuesto</h5>
                    </div>
                </div>
                        ...
                        <tbody>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-hash me-2"></i>Id Impuesto
                                </th>
                                <td class="pe-4">
                                    ${d.id_impuesto || '<span class="text-muted fst-italic">No tiene id impuesto</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-tags me-2"></i>Tipo Impuesto
                                </th>
                                <td class="pe-4">
                                    ${d.tipo_impuesto || '<span class="text-muted fst-italic">No tiene tipo de impuesto</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-percent me-2"></i>Tasa Impuesto
                                </th>
                                <td class="pe-4">
                                    ${d.tasa_impuesto ? d.tasa_impuesto + '%' : '<span class="text-muted fst-italic">No tiene tasa de impuesto</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-chat-left-text me-2"></i>Descripción Impuesto
                                </th>
                                <td class="pe-4" style="max-width: 300px; word-wrap: break-word; white-space: pre-wrap; overflow-wrap: break-word;">
                                    ${d.descr_impuesto || '<span class="text-muted fst-italic">No tiene descripción</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-calendar-plus me-2"></i>Impuesto creado el:
                                </th>
                                <td class="pe-4">
                                    ${d.created_at_impuesto ? formatoFechaEuropeo(d.created_at_impuesto) : '<span class="text-muted fst-italic">Sin fecha</span>'}
                                </td>
                            </tr>
                            <tr>
                                <th scope="row" class="ps-4 w-25 align-top">
                                    <i class="bi bi-calendar-plus me-2"></i>Impuesto actualizado el:
                                </th>
                                <td class="pe-4">
                                  ${d.updated_at_impuesto ? formatoFechaEuropeo(d.updated_at_impuesto) : '<span class="text-muted fst-italic">Sin fecha</span>'}
                                </td>
                            </tr>
                        </tbody>
                    </table>
    ...
    /////////////////////////////////////
    //   INICIO ZONA DELETE IMPUESTOS  //
    ///////////////////////////////////
    function desacImpuesto(id) {
        Swal.fire({
            title: 'Desactivar',
            html: `¿Desea desactivar el impuesto con ID ${id}?<br><br><small class="text-warning"><i class="bi bi-exclamation-triangle me-1"></i>Esto desactivará este tipo de impuesto en el sistema</small>`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.post("../../controller/impuesto.php?op=eliminar", { id_impuesto: id }, function (data) {

                    $table.DataTable().ajax.reload();

                    Swal.fire(
                        'Desactivado',
                        'El impuesto ha sido desactivado',
                        'success'
                    )
                });
            }
    ...
    $(document).on('click', '.desacImpuesto', function (event) {
        event.preventDefault();
        let id = $(this).data('id_impuesto');
        desacImpuesto(id);
    });
    ////////////////////////////////////
    //   FIN ZONA DELETE IMPUESTO    //
    //////////////////////////////////

    ///////////////////////////////////////
    //   INICIO ZONA ACTIVAR IMPUESTO  //
    /////////////////////////////////////
    function activarImpuesto(id) {
        Swal.fire({
            title: 'Activar',
            text: `¿Desea activar el impuesto con ID ${id}?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'Si',
            cancelButtonText: 'No',
            ...
            if (result.isConfirmed) {
                $.post("../../controller/impuesto.php?op=activar", { id_impuesto: id }, function (data) {

                    $table.DataTable().ajax.reload();

                    Swal.fire(
                        'Activado',
                        'El impuesto ha sido activado',
                        'success'
                    )
                });
            }
    ...
    $(document).on('click', '.activarImpuesto', function (event) {
        event.preventDefault();
        let id = $(this).data('id_impuesto');
        console.log("id impuesto:",id);
        
        activarImpuesto(id);
    });
    ////////////////////////////////////
    //   FIN ZONA ACTIVAR IMPUESTO    //
    //////////////////////////////////
    ...
    $(document).on('click', '.editarImpuesto', function (event) {
        event.preventDefault();
        ...
        window.location.href = `formularioImpuesto.php?modo=editar&id=${id}`;
    });
    ///////////////////////////////////////
    //        FIN ZONA EDITAR           //
    /////////////////////////////////////
