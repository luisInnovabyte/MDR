  ...
  if (!document.getElementById("elementos-styles")) {
    ...
    var datatable_elementosConfig = {
        ...
        columns: [
            ...
            // Columna 10: ESTADO GARANTÍA
            { 
                name: 'estado_garantia_elemento',          // <- name para el estado de garantía
                data: 'estado_garantia_elemento',
                className: "text-center",
                render: function(data, type, row) {
                    if (!data) return '<span class="text-muted fst-italic">Sin garantía</span>';
                    let colorClass = data === 'Vigente' ? 'bg-success' :
                                    data === 'Por vencer' ? 'bg-warning text-dark' :
                                    data === 'Vencida' ? 'bg-danger' : 'bg-secondary';
                    return `<span class="badge ${colorClass}">${data}</span>`;
                }
            } ,  
             // Columna 11: FECHA GARANTÍA
            { 
                name: 'fecha_fin_garantia_elemento',           // <- name para la fecha de garantía
                data: 'fecha_fin_garantia_elemento',
                className: "text-center",
                render: function(data, type, row) {
                    return data ? formatoFechaEuropeo(data) : '<span class="text-muted fst-italic">Sin garantía</span>';
                }
            },
            //columna 12: ACTIVO
        ...
        columnDefs: [
            ...
            // Columna 10: estado_garantia_elemento (con color)
            {
                targets: "estado_garantia_elemento:name",
                className: "text-center",
            ...
            // COLUMNA 11 Fecha de garantía
            {
                targets: "fecha_fin_garantia_elemento:name",
                className: "text-center",
                render: function(data) {
    ...
    // Filtrar por estado de garantía
