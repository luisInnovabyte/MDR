...
SELECT
    ...
    -- Tipos de documentos emitidos para este presupuesto (incluye facturas agrupadas)
    NULLIF(CONCAT_WS(',', docs.tipos_documentos, fa_docs.tipos_fa), '') AS tipos_documentos,
    CASE
        WHEN docs.fecha_ultima_factura IS NULL AND fa_docs.fecha_ultima_fa IS NULL THEN NULL
        WHEN docs.fecha_ultima_factura IS NULL THEN fa_docs.fecha_ultima_fa
        WHEN fa_docs.fecha_ultima_fa IS NULL THEN docs.fecha_ultima_factura
        ELSE GREATEST(docs.fecha_ultima_factura, fa_docs.fecha_ultima_fa)
...
-- Subquery: facturas agrupadas vinculadas al presupuesto
LEFT JOIN (
    SELECT
        fap.id_presupuesto,
        GROUP_CONCAT(
            DISTINCT CASE
                WHEN fa.is_abono_agrupada = 1 THEN 'factura_agrupada_abono'
                ELSE 'factura_agrupada'
            END
            ORDER BY fa.is_abono_agrupada ASC
            SEPARATOR ','
        ) AS tipos_fa,
        MAX(fa.fecha_factura_agrupada) AS fecha_ultima_fa
...
) fa_docs ON fa_docs.id_presupuesto = p.id_presupuesto
